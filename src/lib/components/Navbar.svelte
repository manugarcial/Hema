<script lang="ts">
  ...
  let cartItems = get(cart); // Initial cart state

  // Subscribe to cart updates and calculate the total sum
  let totalSum = 0; // Variable to hold the total price

  // Update totalSum when cart items change
  cart.subscribe((items) => {
    cartItems = items;
    totalSum = cartItems.reduce((sum, item) => sum + item.quantity, 0);
  });
</script>
  ...
  <ul class="navbar">
    ...
    <li>
      <a href="/cart" class="cart-icon">
        <svg fill="#000000" class="cart-image" width="40px" height="40px" viewBox="0 0 64 64" enable-background="new 0 0 64 64" id="Layer_1" version="1.1" xml:space="preserve" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink">
